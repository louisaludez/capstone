function updateCardFieldsCheckin() {
  const paymentMethodCheckin = document.getElementById("payment-method");
  const cardFields = document.querySelectorAll(".card-fields");
  if (paymentMethodCheckin.value === "credit-card") {
...
const logoURLCheckin =
  document.getElementById("checkin").dataset.logoUrlCheckin;
var modalCheckin = document.getElementById("checkin");
var btn = document.getElementById("check-in-modal");
if (btn) {
...
document.addEventListener("DOMContentLoaded", function () {
  const paymentMethodCheckin = document.getElementById("payment-method");
  updateCardFieldsCheckin();
  paymentMethodCheckin.addEventListener("change", updateCardFieldsCheckin);
});
...
function startCheckInTimer(duration, display) {
  ...
  checkInTimerInterval = setInterval(function () {
    ...
    if (--timer < 0) {
      ...
      modalCheckin.style.display = "none"; // Close the modal when time is up
      Swal.fire({
